    ...
    **kwargs: :class:`Any`
    ...
    def __init_subclass__(
        ...
        **kwargs: typing.Any,
        ...
        for method_name in itertools.filterfalse(
            ...
            if not hasattr(
                (method := getattr(cls, method_name)), '__progress_ignored__'
            ):
            ...
            methods: typing.List[str] = [
                m for m in filter(
                    lambda m: all((
                        ...
                        hasattr(cls, f'__{cls.__name__}_{m}_callback__'),  # Checking if the method uses @ignored.
                    )),
                    dir(cls)
                )
            ]
    ...
    def copy(cls: typing.Type[Progress], reference: str) -> None:
        ...
        for dunder in filter(lambda m: m.endswith(f'{cls.__name__}_{reference}'), dir(cls)):
            ...
            setattr(cls, f'{dunder[:dunder.find("__", 1) + 2]}{unique}', getattr(cls, dunder))
