...
def test_progressbar_and_sectors() -> None:
    ...
    assert_that(progressbar.storage[0].name, equal_to(sector_name))

    new_sector_name: typing.Literal["new_name"] = "new_name"

    progressbar.replace_visual(sector_pos, new_sector_name)
    assert_that(progressbar.storage[0].name, not_(equal_to(sector_name)))
    assert_that(progressbar.storage[0].name, equal_to(new_sector_name))
